(function() {
  'use strict';
  var Glob, checkIfValid, colors, confirmWithUser, find, findSync, fs, getConfigs, list, main, path, printHelp, readline, removeDirectly, removeWithConfirmation, rename, yargs, _,
    __slice = [].slice;
  ...
  main = function(argv) {
    ...
    if (isConfigsValid) {
      ...
      if (configs.help) {
        ...
        return find(configs.destination, configs.extnames, list);
      } else if (configs.remove) {
        return findSync(configs.destination, configs.extnames, removeWithConfirmation, configs.verbose);
      } else if (configs.removeDirectly) {
        return findSync(configs.destination, configs.extnames, removeDirectly, configs.verbose);
      } else {
        return find(configs.destination, configs.extnames, rename, configs.renameTo, configs.verbose);
      }
    } else {
      messages.forEach(function(msg) {
